...
function validateTime(obj) {
  ...
  /* 
  while(obj.value.length<2)
   	  obj.value = ("0"+obj.value);
  */
 }
 
 function mctr_updatetime(obj,name)
 {
    var tmp="";     
    for (var i=0;i<document.getElementsByName(name).length;i++) {
    	while(document.getElementsByName(name)[i].value.length<2)
   		{
   	   		document.getElementsByName(name)[i].value= ("0"+document.getElementsByName(name)[i].value);
   		}
    	tmp+=document.getElementsByName(name)[i].value;
    	if(i!=document.getElementsByName(name).length-1)
    	   tmp+=":";
   } 
   obj.value=tmp;
 }
 
 function setTimeFocusObj(obj,fName) {
   document.getElementById(fName+"_up").disabled=document.getElementById(fName+"_down").disabled=false;
   obj.select();
   zx_timeObj=obj;
 }
 
 function changeTime(obj,name,interval)
 {
    if (!zx_timeObj) return;
    if(zx_timeObj.name!=name)
       return;
    var cmd = event.srcElement.innerText=="5"?true:false;
    var i = parseInt(zx_timeObj.value,10);
    var radix = parseInt(zx_timeObj.radix,10);
    if(radix==24)
       interval=1;
    ...
    if(i<=0)
     i+=interval;
    	...
    	i-=interval;
    zx_timeObj.value = i;
    var tmp=eval(zx_timeObj.radix-1);
    var tmpStr=new String(tmp);
  	
    while(zx_timeObj.value.length<tmpStr.length)
    {
   		zx_timeObj.value = ("0"+zx_timeObj.value);
    }
    mctr_updatetime(obj,name)
